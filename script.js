...
const actualizarMensaje = () => {
  ...
  ////Toma los parametros de las otra funcion
  fullparking(espaciosLibres, celdasVacias);
};


//Funcion para actualizar el mensaje
const fullparking = (espaciosLibres, celdasVacias) => {
    const buttonadd = document.querySelector('form button');

    if(celdasVacias.length === 0){
        buttonadd.disabled = true;
        input.disabled = true;
        espaciosLibres.innerText = 'Parqueo Libre';
        espaciosLibres.style.color = "red";
    } else if(celdasVacias.length === 1){
        espaciosLibres.innerText = `Tiene ${celdasVacias.length} parqueo libre`;
        espaciosLibres.style.color = "";
        buttonadd.disabled = false;
        input.disabled = false;
    } else {
        //Para deshabilitar el input y el button de agregar!
        espaciosLibres.innerText = `Tienes ${celdasVacias.length} parqueos libres`;
        espaciosLibres.style.color = "";
        buttonadd.disabled = false;
        input.disabled = false;
    }
